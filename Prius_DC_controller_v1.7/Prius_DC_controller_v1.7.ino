...
// VERSION 1070 23/07/2021 ( changed current limiting implimentation to be include a level of being proportional to throttle pedal helps aviod invertor shutdown due to asking for too many amps at lower pwm levels) 
...
int cl_high = 800; //upper current limit  
const int dt = 60; //Temperature threshold for overtemp
...
if (filtered_volts <= 256) {error=1; fail();}
...
{   
  ...
  if (filtered_throttle>pwm_duty && current_limit > mg_amps) { pwm_duty++; pwm_duty++;} 
...
void Reverse() {  
  ...
  if (filtered_throttle>pwm_duty && mg_amps <= 100) pwm_duty++; 
  if (filtered_throttle<pwm_duty) pwm_duty--;
...
Serial.println ("");
 ...
 if (timer1 >= 1500) { timer1 = timer1 - 1500;    
